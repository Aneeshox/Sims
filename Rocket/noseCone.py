...
class NoseCone(FixedMass, BodyComponent):
    ...
    def _getVolume(self):
        ...
        if self.shape == "tangentOgive":
            ...
            return 0.00548694432944#0.00518649458172 #Aneesh
    ...
    def _getPlanformArea(self):
        ...
        if self.shape == "tangentOgive":
            ...
            return 0.0544855289258 #0.0252965968796*2
    ...
    def _getWettedArea(self):
        ...
        if self.shape == "tangentOgive":
            ...
            return 0.172892873615#0.160929230758
    ...
    def getRadius(self, distanceFromTip: float) -> float:
        if self.shape == "cone":
            ...
            theta = math.acos(1-(2*distanceFromTip/(self.baseDiameter*self.aspectRatio)))
            firstBit = self.baseDiameter / (2*math.sqrt(math.pi))
            return firstBit*math.sqrt(theta - (math.sin(2*theta)/2))
        elif self.shape =="Aegis Von Karman":
            theta = math.acos(1-(2*distanceFromTip/(self.baseDiameter*self.aspectRatio)))
            firstBit = self.baseDiameter / (2*math.sqrt(math.pi))
            return firstBit*math.sqrt(theta - (math.sin(2*theta)/2))
